...
describe('workspace narrator e2e', () => {
  it(
    'relays narrator messages through the backend server',
    { timeout: 60_000 },
    async () => {
      ...
      try {
        const postResult = await requestJson<{ workspaceId: string; conversationId: string; eventId: string; taskId: string }>(
...
async function requestJson<T>(url: string, init: { method: string; body?: Record<string, unknown> }): Promise<JsonResponse<T>> {
