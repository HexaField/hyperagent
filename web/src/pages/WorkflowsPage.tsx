    ...
    <div class="flex flex-col gap-6">
      <header class="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p class="text-[var(--text-muted)]">Inspect orchestration progress, jump into steps, and switch to repository graphs for deeper context.</p>
        </div>
        <div class="flex gap-3">
          <A href="/launch" class="rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white">Launch workflow</A>
      ...
      <Show when={workflows()} fallback={<p class="text-sm text-[var(--text-muted)]">Loading workflows…</p>}>
        {items => (
          <Show when={items().length} fallback={<p class="text-sm text-[var(--text-muted)]">No workflows queued yet.</p>}>
                  ...
                  <article class="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-4">
                    <header class="mb-4 flex flex-wrap items-center justify-between gap-3">
                      ...
                      <div class="flex gap-2 text-sm">
                        <A href={`/workflows/${item.workflow.id}`} class="rounded-xl border border-blue-600 px-3 py-1.5 text-blue-600">
                          Inspect
                        </A>
                      </div>
                    </header>
                        ...
                        {step => (
                          <li class="rounded-xl border border-[var(--border)] bg-[var(--bg-muted)] p-3">
                            <div class="flex flex-wrap items-center justify-between gap-3">
                              <p class="font-semibold text-[var(--text)]">{typeof step.data?.title === 'string' ? (step.data.title as string) : `Step ${step.sequence}`}</p>
