...
- [x] Add comprehensive automated tests.
	- Unit tests: cover log serialization/deserialization, task graph CRUD/locking, prompt builders, and StreamingLLM client wrappers (Vitest + Pytest suites scoped to this module).
	- Workflow tests: run orchestrator + sidecar in-process to simulate multi-agent flows, verifying event log integrity, gating decisions, and narrator outputs.
	- E2E tests: reuse existing `tests/e2e/workflow.e2e.test.ts` patterns but pointed at the sidecar endpoints, ensuring the UI can stream narrator tokens end-to-end.
	- CI gating: add module-specific `npm run test:streaming-llm` and `pytest streaming-llm/backend` steps so regressions block merges touching this folder.
- [x] Perform load/performance testing and hardening.
	- Stress harness: synthetic workload runner spawns multiple concurrent conversations with long-running tasks to measure log write throughput, latency, and orchestrator CPU usage.
	- KV cache validation: benchmark local HF models with `STREAMING_LLM_ENABLE=1/0` to ensure streaming optimizations behave as expected; capture metrics in `data/benchmarks/`.
	- Retention policies: cron/script rotates JSONL logs and compacts summaries; add guardrails to cap disk usage and alert if storage exceeds thresholds.
	- Fault injection: introduce agent crashes/timeouts during load tests to verify retry + narrator fallback logic holds under pressure.
- [x] Plan rollout, security, and privacy validation.
	- Shadow mode: run orchestrator + sidecar in parallel with existing system, mirroring traffic but keeping narrator output hidden; compare decisions/logs to baseline before promotion.
	- Traffic splits: once validated, expose to small % of users via reverse proxy routing (`streaming-llm-canary` upstream) and monitor key metrics (latency, error rate, narrative quality).
	- Security review: ensure agents only access approved data, verify log storage isolation, and run dependency scans on the sidecar (pip/npm audit) prior to release.
	- Privacy controls: add configurable retention window + redaction hooks so sensitive data can be purged; document compliance procedures within this module’s README.
